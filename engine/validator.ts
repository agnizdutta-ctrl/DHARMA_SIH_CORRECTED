...
export class DharmaEngine {
  ...
  isValidMove(
    ...
    if (isTopAadesh) {
      ...
      return {
        valid: false,
        reason: `Krishna's Aadesh requires a ${declared} category card or a Wild card`
      };
    }

    // If topCard is Crisis: Lakshagriha (House of Lac) → forces next player to play Region (R) or Wild
    const isCrisisLakshagriha =
      targetCard.type === 'crisis' &&
      (targetCard.effect === 'force_region' ||
        targetCard.name.toLowerCase().includes('lakshagriha') ||
        targetCard.name.toLowerCase().includes('lac') ||
        targetCard.name.toLowerCase().includes('famine'));

    if (isCrisisLakshagriha) {
      if (playedCat === 'R') {
        return {
          valid: true,
          reason: 'Region (R) card satisfies the Lakshagriha Crisis escape',
          matchType: 'crisis_match'
        };
      }
      return {
        valid: false,
        reason: 'Crisis: Lakshagriha requires an escape to a Region (R) card or a Wild card'
      };
    }

    // If topCard is Crisis: Kurukshetra (Great War) → forces next player to play Marvel (M) or Wild
    const isCrisisKurukshetra =
      targetCard.type === 'crisis' &&
      (targetCard.effect === 'force_marvel' ||
        targetCard.name.toLowerCase().includes('kurukshetra') ||
        targetCard.name.toLowerCase().includes('war') ||
        targetCard.name.toLowerCase().includes('nadir'));

    if (isCrisisKurukshetra) {
      if (playedCat === 'M') {
        return {
          valid: true,
          reason: 'Marvel (M) card satisfies the Kurukshetra Crisis defense',
          matchType: 'crisis_match'
        };
      }
      return {
        valid: false,
        reason: 'Crisis: Kurukshetra requires a divine weapon / Marvel (M) card or a Wild card'
      };
    }

    // If topCard is other Wild (Shakuni, Ashwatthama, Mouna, Chakravyuha) → any card can be played next
