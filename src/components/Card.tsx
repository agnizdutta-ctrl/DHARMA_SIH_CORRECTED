...
export const Card: React.FC<CardProps> = ({
  ...
  const getCategoryMeta = () => {
    ...
    if (cat === 'C') {
      return {
        symbol: card.symbol || '👑',
        label: currentEra === 'mughal' ? 'PADISHAH' : currentEra === 'swaraj' ? 'VANGUARD' : 'CREATOR',
        color: '#D4AF37',
        badgeBg: 'bg-[#B7950B]',
    ...
    if (cat === 'Le' || cat === 'L') {
      return {
        symbol: card.symbol || (currentEra === 'swaraj' ? '✊' : currentEra === 'mughal' ? '📜' : '⚖️'),
        label: currentEra === 'mughal' ? 'LAW' : currentEra === 'swaraj' ? 'MOVEMENT' : 'DHARMA',
        color: '#8E44AD',
        badgeBg: 'bg-[#8E44AD]',
        tagText: 'text-purple-300'
      };
    ...
    if (cat === 'Ma' || cat === 'M') {
      return {
        symbol: card.symbol || (currentEra === 'swaraj' ? '⚙️' : currentEra === 'mughal' ? '💎' : '🏛️'),
        label: currentEra === 'swaraj' ? 'CONTRIBUTION' : 'MARVEL',
        color: '#2980B9',
        badgeBg: 'bg-[#2471A3]',
    ...
    if (cat === 'R') {
      return {
        symbol: card.symbol || (currentEra === 'swaraj' ? '📍' : currentEra === 'mughal' ? '🕌' : '🗺️'),
        label: currentEra === 'mughal' ? 'SUBAH' : currentEra === 'swaraj' ? 'TERRITORY' : 'REGION',
        color: '#27AE60',
        badgeBg: 'bg-[#229954]',
  ...
  return (
    ...
    >
      ...
      {/* Authentic Ancient Epic Canvas */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#2E1D0E] via-[#20140A] to-[#120A04] pointer-events-none" />
      ...
      <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-60 bg-[#0D241A] text-[#F5EBE6] p-3 rounded-xl shadow-2xl text-[11px] opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 z-50 border border-[#D4AF37]/60">
        ...
        <p className="text-white/85 text-[10px] leading-relaxed">
          {card.detail || card.historicalDetail || card.historicalContext || 'Sacred chronicle from the great epic Mahabharat.'}
        </p>
        {card.effect && (
