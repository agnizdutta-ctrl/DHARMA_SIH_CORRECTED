...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'ok',
    game: 'Dharma: Unified Chronicles of Bharat',
    activeRooms: roomManager.getAllRoomsCount()
  });
});

app.get('/api/cards', (req, res) => {
  res.json(cardsMahabharat);
});

app.get('/api/cards/:era', (req, res) => {
  const { era } = req.params;
  if (era === 'mughal') return res.json(cardsMughal);
  if (era === 'swaraj') return res.json(cardsSwaraj);
  return res.json(cardsMahabharat);
});

// Socket.IO handling
io.on('connection', (socket) => {
  socket.on('room:create', ({ roomId, maxRounds, playerName, p2Name, againstAI, eraId }) => {
    const rId = roomId || `court_${Math.random().toString(36).substr(2, 6)}`;
    let room = roomManager.getRoom(rId);
    if (!room) {
      room = roomManager.createRoom(rId, maxRounds || 7, eraId || 'mahabharat');
    }
...
async function start() {
  ...
  server.listen(PORT, '0.0.0.0', () => {
    console.log(`Dharma: Mahabharat server running on http://0.0.0.0:${PORT}`);
  });
}
